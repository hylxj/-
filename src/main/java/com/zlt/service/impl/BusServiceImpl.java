...
public class BusServiceImpl implements BusService {
    ...
    public String addUploadBus(UploadBus bus) {

        Map<String, Integer> map = busDriverDao.findIdByname(bus.getBusdriverName());
        Map<String, Integer> map1 = busLineDao.findIdByName(bus.getLineName());
        Bus bus1 = new Bus();
        bus1.setBusPlate(bus.getBusPlate());
        bus1.setBusName(bus.getBusName());
        bus1.setBusCreateTime(bus.getBusCreateTime());
        bus1.setBusStatus(bus.getBusStatus());
        if (map == null || map.isEmpty()) {
            return "导入失败，系统没有司机" + bus.getBusdriverName() + "的信息";
        }
        if (map1 ==  null || map1.isEmpty()) {
            return "导入失败，系统没有线路" + bus.getBusdriverName() + "的信息";
        }
        bus1.setBusDriverId(map.get("busdriverId"));
        bus1.setBusLineId(map1.get("lineId"));
        busDao.saveBus(bus1);
        return "导入成功";
    }
}
